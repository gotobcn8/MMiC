...
def generate_augmentation(dir_path,num_clients,num_classes,niid,balance,partition,alpha):
    if not os.path.isabs(dir_path):
        dir_path = os.path.join('repository',dir_path)
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    ...
    trainset = torchvision.datasets.CIFAR10(
        root=dir_path+"rawdata", train=True, download=True, transform=train_transform)
    testset = torchvision.datasets.CIFAR10(
        root=dir_path+"rawdata", train=False, download=True, transform=test_transform)
    trainloader = torch.utils.data.DataLoader(
