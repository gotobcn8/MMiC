...
def tokenize(sentence, vocab, caption_drop_prob):
    ...
    if caption_drop_prob > 0:
        unk = vocab("<unk>")
        tokenized = [
            vocab(token) if random.random() > caption_drop_prob else unk
            for token in tokens
        ]
    else:
        tokenized = [vocab(token) for token in tokens]
...
def caption_transform(vocab, caption_drop_prob=0):
    ...
    transform.append(
        partial(tokenize, vocab=vocab, caption_drop_prob=caption_drop_prob)
    )
...
def imagenet_transform(
    ...
    if custom_transforms is not None:
        if not isinstance(custom_transforms, list):
            raise TypeError(
                f"custom_transforms should be list, not {type(custom_transforms)}"
            )
    transform = []
