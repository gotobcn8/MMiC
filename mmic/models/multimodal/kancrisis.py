...
class MMModel(BaseModel):
    def __init__(self, imageEncoder =None, textEncoder = None, save_dir = None):
...
class DenseNetBertMMModel(MMModel):
    ...
    def __init__(self, save_dir = None, dim_visual_repr = 1280, dim_text_repr=768, dim_proj=100, num_class=2):
        self.save_dir = save_dir
        ...
        super(DenseNetBertMMModel, self).__init__(None, None, save_dir)
        ...
        self.v_contrast_layer = nn.Linear(in_features = 100,out_features = 100, bias = False)
        self.t_contrast_layer = nn.Linear(in_features = 100,out_features = 100, bias = False)
        self.joint_contrast_layer = nn.Linear(in_features = 100,out_features = 100, bias = False)
        self.joint_contrast_layer_plus = nn.Linear(in_features = 100,out_features = 100, bias = False)
        self.share_encode = nn.Linear(in_features = 100,out_features = 100, bias = False)
        self.shrink_layer = nn.Linear(in_features = 200,out_features = 100, bias = False)
        
    def shared_encoder(self,x):
        return F.normalize(self.share_encode(x),dim = -1)
        
    def get_batch_representations(self,vr,tr):
        vr = self.v_contrast_layer(vr)
        tr = self.t_contrast_layer(tr)
        return [self.shared_encoder(vr),self.shared_encoder(tr)]
    
    def forward(self, x, kwargs = None):
        image, text , len_i, len_t = x
        ...
        text = self.dropout(text[:,0,:])  # N, dim_text_repr
        ...
        text = text.squeeze(1)
        f_i_self_attn = self.apply_self_attention(image)  # N, dim_proj
        e_i_self_attn = self.apply_self_attention(text)  # N, dim_proj
        
        # Getting linear projections (eqn. 3)
        proj_v = self.proj_visual(f_i_self_attn)
        f_i_tilde = F.relu(self.proj_visual_bn(proj_v))  # N, dim_proj
        ...
        batch_representations = self.get_batch_representations(alpha_v_i,alpha_e_i)
        ...
        batch_representations.append(joint_repr)
        output = self.joint_contrast_layer_plus(F.dropout(F.relu(self.joint_contrast_layer(joint_repr)), p=0.0, training=self.training))
        output += joint_repr
        
        # Get class label prediction logits with final fully-connected layers
        return self.cls_layer(output), batch_representations